            ...
            for barcode in barcodes:
                ...
                if verify_encoded_username(extracted_username, extracted_hash, my_time):
                    ...
                    return redirect(
                        url_for(
                            "student_portal",
                            message=f"Attendance marked for lecture by {professor_name}",
                        )
                    )
                else:
                    return redirect(
                        url_for(
                            "student_portal",
                            error="QR code does not match any stored user data.",
                        )
                    )
        else:
...
def student_portal():
    ...
    return render_template(
        "student_portal.html", logs=logs, message=message, error=error
    )
